...
export class SearxngSearchProvider {
    ...
    async _searchPageViaDocker({ query, language, categories, recencyHours, pageno, limit } = {}) {
        if (!this.dockerFallbackContainer) throw new Error('SearXNG docker_fallback_container 未配置');
        const timeRange = toTimeRange(recencyHours);
        const url = this._buildSearchUrl({ query, language, categories, timeRange, pageno });
    ...
    async search({ query, language, categories = 'general', recencyHours = 24, limit = 10 } = {}) {
        ...
        try {
            ...
            return this._searchPageViaDocker({ query: q, language, categories, recencyHours, pageno: 1, limit });
        }
    }
        ...
        for (let p = 1; p <= Math.min(pages, 5); p++) {
            try {
                ...
                try {
                    ...
                    pageResults = await this._searchPageViaDocker({
                        query: q, language, categories, recencyHours, pageno: p, limit: resultsPerPage,
                    });
                }
