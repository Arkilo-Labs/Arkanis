...
export default function SetupPage({ setupToken }) {
  ...
  const validationMessage = useMemo(() => {
    ...
    if (!trimmedUsername) return '请输入用户名或邮箱（3-64 位）';
    if (!USERNAME_REGEX.test(trimmedUsername))
      return '用户名须 3-64 位，支持字母/数字/._@-，首尾须为字母或数字';
  ...
  return (
    <div className="auth-card">
      <div className="mb-6">
        ...
        <p className="text-sm text-text-muted mt-2">首次部署需要创建管理员账号，用于后续登录。</p>
      </div>
      ...
      <form className="space-y-4" onSubmit={submit}>
        <div>
          ...
          />
          <p className="form-hint">支持邮箱或用户名，3-64 位。</p>
        </div>
        ...
        {!error && !canSubmit ? <p className="form-hint">{validationMessage}</p> : null}
        ...
        <button className="btn btn-primary btn-full" disabled={isSubmitting} type="submit">
          <i className={isSubmitting ? 'fas fa-spinner fa-spin' : 'fas fa-wrench'}></i>
          完成初始化
        </button>
      </form>
    </div>
