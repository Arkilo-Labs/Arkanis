  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open ? (
        ...
        >
          {/* 第零级：全量 */}
          <button type="button" className="use-dropdown-item" onClick={() => trigger('all')}>
          ...
          >
            ...
            <i
              className={[
                'fas ml-auto text-[10px]',
                rtExpanded ? 'fa-chevron-up' : 'fa-chevron-down',
              ].join(' ')}
            ></i>
          </button>
          {rtExpanded ? (
          ...
          >
            ...
            <i
              className={[
                'fas ml-auto text-[10px]',
                lensExpanded ? 'fa-chevron-up' : 'fa-chevron-down',
              ].join(' ')}
            ></i>
          </button>
          {lensExpanded ? (
...
function AddProviderModal({ onClose, onCreate }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.name.trim() || !form.baseUrl.trim() || !form.modelName.trim()) {
      setError('名称、Base URL、Base Model Name 为必填项');
      return;
    }
    if (!form.protocol) {
      setError('API 协议为必填项');
      ...
      <div className="modal-card max-w-2xl p-8" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            >
              <i
                className={['fas text-xs', showAdvanced ? 'fa-chevron-up' : 'fa-chevron-down'].join(
                  ' '
                )}
              ></i>
              高级设置
...
export default function AIProvidersTab() {
  ...
  const [lensConfig, setLensConfig] = useState({
    roles: { lens: null, newser: null, researcher: null, auditor: null },
  });
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setLensConfig(
        lensData || { roles: { lens: null, newser: null, researcher: null, auditor: null } }
      );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-muted mt-2">管理 Provider 定义与密钥</p>
        </div>
      ...
      {/* 添加 Provider 弹窗 */}
      {showAddModal ? (
        <AddProviderModal onClose={() => setShowAddModal(false)} onCreate={createProvider} />
      ) : null}
    ...
    <div className="space-y-3">
      ...
      <div className="card p-4 space-y-4">
        ...
        {rtMainAgents.length > 0 ? (
          <RoleSection
            label="Roundtable"
            agents={rtMainAgents.map((a) => ({
              type: 'rt',
              name: a.name,
              role: a.role,
              defaultRef: a.defaultProviderRef,
            }))}
        ...
        {lensRoleKeys.length > 0 ? (
          <RoleSection
            label="Lens"
            agents={lensRoleKeys.map((k) => ({
              type: 'lens',
              name: k,
              role: LENS_ROLE_LABELS[k] || k,
            }))}
            expandedKey={expandedKey}
            onToggle={toggle}
            getDraft={getDraft}
  ...
  return (
    <div className="space-y-1.5">
      ...
      {agents.map((agent) => {
        ...
        const effectivePid =
          agent.type === 'rt' ? (draftPid ?? agent.defaultRef) : (draftPid ?? null);
              ...
              <span className="flex items-center gap-2 flex-shrink-0">
                {currentProvider ? (
                  <span
                    className={[
                      'badge text-[10px]',
                      isOverridden ? 'badge-accent' : 'badge-muted',
                    ].join(' ')}
                  >
