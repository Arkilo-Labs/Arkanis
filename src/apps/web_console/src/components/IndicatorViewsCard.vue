<template>
    <div v-if="views" class="iv-card">
        <div class="iv-header">
            <div class="iv-title">
                <span class="iv-title-text">{{ title }}</span>
            </div>
            <span class="iv-subtitle">AI 观点</span>
        </div>

        <div class="iv-body">
            <div v-for="item in items" :key="item.key" class="iv-row">
                <div class="iv-row-top">
                    <span class="iv-left">{{ item.name }}</span>
                    <span :class="item.badgeClass">{{ item.right }}</span>
                </div>
                <div v-if="item.note" class="iv-note">{{ item.note }}</div>
            </div>
        </div>
    </div>
</template>
...
const props = defineProps({
    ...
    title: { type: String, default: '指标观点' },
});
...
const items = computed(() => {
    ...
    return [
        { key: 'macd', name: 'MACD', right: biasLabel(macdBias), badgeClass: badgeClassByBias(macdBias), note: v?.macd?.note || '' },
        { key: 'boll', name: '布林带', right: biasLabel(bollBias), badgeClass: badgeClassByBias(bollBias), note: v?.bollinger?.note || '' },
        {
            key: 'trend',
            name: '趋势强度',
        ...
        { key: 'overall', name: '综合倾向', right: biasLabel(overallBias), badgeClass: badgeClassByBias(overallBias), note: '' },
    ];
});
</script>
